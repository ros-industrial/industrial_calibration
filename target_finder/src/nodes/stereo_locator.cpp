...
StereoLocatorService::StereoLocatorService(ros::NodeHandle nh)
{
  ...
  ROS_INFO("left camera info focal:%f %f center:%f %f  radial:%f %f %f tang: %f %f", lfx_, lfy_, lcx_, lcy_, lk1_, lk2_,
           lk3_, lp1_, lp2_);

  ROS_INFO("right camera info focal:%f %f center:%f %f  radial:%f %f %f tang: %f %f", rfx_, rfy_, rcx_, rcy_, rk1_,
           rk2_, rk3_, rp1_, rp2_);
...
bool StereoLocatorService::getPoseCallBack(target_finder::stereo_locator::Request& req,
                                           target_finder::stereo_locator::Response& res)
{
  ...
  if (left_ok && right_ok)
  {
    ...
    {  // add a new cost to the problem for each observation
      for (int j = 0; j < target_->num_points_; j++)
      {
        ...
        cost_function[j] = industrial_extrinsic_cal::StereoTargetLocator::Create(
            left_image_x, left_image_y, left_point, right_image_x, right_image_y, right_point, LCtoRC, lfx_, lfy_, lcx_,
            lcy_, lk1_, lk2_, lk3_, lp1_, lp2_, rfx_, rfy_, rcx_, rcy_, rk1_, rk2_, rk3_, rp1_, rp2_);
  ...
  if (summary.termination_type != ceres::NO_CONVERGENCE)
  {
    ...
    if (final_cost <= req.allowable_cost_per_observation)
    {
      ...
      target_->pose_.getQuaternion(res.target_pose.orientation.x, res.target_pose.orientation.y,
                                   res.target_pose.orientation.z, res.target_pose.orientation.w);
      return true;
    }
    else
    {
