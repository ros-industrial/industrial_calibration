...
public:
  ...
  bool pose_move(const industrial_extrinsic_cal::robot_pose_triggerGoalConstPtr& goal)
  {
    ...
    config.start_waypoint_ =
        std::make_shared<tesseract_motion_planners::JointWaypoint>(joint_start->position, joint_start->name);
    Eigen::Vector3d position(goal->pose.position.x, goal->pose.position.y, goal->pose.position.z);
    Eigen::Quaterniond quat(goal->pose.orientation.x, goal->pose.orientation.y, goal->pose.orientation.z,
    ...
    config.link_ =
        config.tesseract_->getFwdKinematicsManagerConst()->getFwdKinematicSolver(manipulator_name_)->getTipLinkName();
    ...
    // here's the signature: inline void toMsg(trajectory_msgs::JointTrajectory& traj_msg, const
  ...
  bool joint_move(const industrial_extrinsic_cal::robot_joint_values_triggerGoalConstPtr& goal)
  {
    ...
    {
      ROS_ERROR("# of joints of goal != # of joints of robot %d != %d", (int)joint_start->position.size(),
                (int)goal->joint_values.size());
      return (false);
    }
    ...
    config.end_waypoint_ =
        std::make_shared<tesseract_motion_planners::JointWaypoint>(goal->joint_values, joint_start->name);
    config.num_steps_ = 20;
    config.tesseract_ = env_monitor_->getTesseractConst();
    config.link_ =
        config.tesseract_->getFwdKinematicsManagerConst()->getFwdKinematicSolver(manipulator_name_)->getTipLinkName();
