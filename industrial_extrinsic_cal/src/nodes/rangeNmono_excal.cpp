...
public:
  ...
  bool executeCallBack(industrial_extrinsic_cal::calibrate::Request& req,
  ...
  : action_server_(nh_, "run_calibration", boost::bind(&RangeNmonoExCalService::actionCallback, this, _1), false)
{
  ...
  if (pnh.getParam("range_ROI_xmin", range_roi_.x_min) && pnh.getParam("range_ROI_ymin", range_roi_.y_min) &&
      pnh.getParam("range_ROI_xmax", range_roi_.x_max) && pnh.getParam("range_ROI_ymax", range_roi_.y_max))
  {
    has_rroi_ = true;
  }
  else
  ...
  if (pnh.getParam("mono_ROI_xmin", mono_roi_.x_min) && pnh.getParam("mono_ROI_ymin", mono_roi_.y_min) &&
      pnh.getParam("mono_ROI_xmax", mono_roi_.x_max) && pnh.getParam("mono_ROI_ymax", mono_roi_.y_max))
  {
    has_mroi_ = true;
  }
  else
  ...
  range_camera_ = shared_ptr<industrial_extrinsic_cal::Camera>(
      new industrial_extrinsic_cal::Camera(range_camera_name_, range_cp, false));
  mono_camera_ = shared_ptr<industrial_extrinsic_cal::Camera>(
      new industrial_extrinsic_cal::Camera(mono_camera_name_, mono_cp, false));
  initMonoCamera();
  initRangeCamera();
                                             ...
                                             industrial_extrinsic_cal::calibrate::Response& res)
{
  ...
  for (int i = 0; i < num_range_observations; i++)
  {
    ...
    // therefore the position in x,y,z of this point in space must be interpolated between the four surrounding point
    // cloud points
    ...
    int height =
        cloud->height;  // here we are assuming the height and width of the image and the point cloud are identical
    ...
    ROS_INFO("image(%f %f) pt3(%f %f %f), tpt(%f %f %f)", image_x, image_y, pt3.x, pt3.y, pt3.z, tpoint.x, tpoint.y,
             tpoint.z);
...
void RangeNmonoExCalService::initMonoCamera()
{
  ...
  mono_camera_->camera_observer_ =
      shared_ptr<ROSCameraObserver>(new ROSCameraObserver(mono_image_topic_, mono_camera_name_));
...
void RangeNmonoExCalService::initRangeCamera()
{
  ...
  shared_ptr<industrial_extrinsic_cal::TransformInterface> range_ti =
      shared_ptr<industrial_extrinsic_cal::ROSSimpleCameraCalTInterface>(
          new industrial_extrinsic_cal::ROSSimpleCameraCalTInterface(range_camera_frame_,
                                                                     range_camera_mounting_frame_));
  std::string ref_frame("dummy");
  range_ti->setReferenceFrame(ref_frame);
  range_camera_->setTransformInterface(range_ti);

  // create the observers and get the camera info data if available, we only use it to set ROI when not set by rosparam
  range_camera_->camera_observer_ =
      shared_ptr<ROSCameraObserver>(new ROSCameraObserver(range_image_topic_, range_camera_name_));
  double fx, fy, cx, cy, k1, k2, k3, p1, p2;
  int width, height;
