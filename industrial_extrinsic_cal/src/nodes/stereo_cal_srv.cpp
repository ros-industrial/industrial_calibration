...
public:
  ...
  bool observationCallBack(std_srvs::TriggerRequest& req, std_srvs::TriggerResponse& res)
  {
    ...
    all_cameras_[0]->camera_observer_->addTarget(all_targets_[0], roi,
                                                 industrial_extrinsic_cal::cost_functions::WristStereoCal);
    ...
    all_cameras_[1]->camera_observer_->addTarget(all_targets_[0], roi,
                                                 industrial_extrinsic_cal::cost_functions::WristStereoCal);
    ...
    if (left_camera_observations.size() == all_targets_[0]->num_points_ &&
        right_camera_observations.size() == all_targets_[0]->num_points_)
    {
      ROS_INFO("Found %d left camera observations and %d right camera observations",
               (int)left_camera_observations.size(), (int)right_camera_observations.size());
      ...
      for (int k = 0; k < all_targets_[0]->num_points_; k++)
      {
        ...
        CostFunction* cost_function = industrial_extrinsic_cal::WristStereoCal::Create(
            left_image_x, left_image_y, left_point, right_image_x, right_image_y, right_point, lfx, lfy, lcx, lcy, rfx,
            rfy, rcx, rcy);
    ...
    */
    ...
    if (summary.termination_type != ceres::NO_CONVERGENCE)
    {
      ...
      else
      {
        ...
        Pose6d P(
            all_cameras_[1]->camera_parameters_.pb_extrinsics[3], all_cameras_[1]->camera_parameters_.pb_extrinsics[4],
            all_cameras_[1]->camera_parameters_.pb_extrinsics[5], all_cameras_[1]->camera_parameters_.pb_extrinsics[0],
            all_cameras_[1]->camera_parameters_.pb_extrinsics[1], all_cameras_[1]->camera_parameters_.pb_extrinsics[2]);
  ...
  bool loadCallBack(industrial_extrinsic_cal::FileOp::Request& req, industrial_extrinsic_cal::FileOp::Response& resp)
  {
    ...
    while (data_read_ok)
    {  // read all the scene data from image_directory
      ...
      if (data_read_ok)
      {
            ...
            right_camera_observations.size() == all_targets_[0]->num_points_)
        {
          ...
          shared_ptr<Target> target = left_camera_observations[0].target;
          ceres_blocks_.addMovingTarget(all_targets_[0], scene_);
          P_BLOCK left_intrinsics =
              ceres_blocks_.getStaticCameraParameterBlockIntrinsics(all_cameras_[0]->camera_name_);
          P_BLOCK right_intrinsics =
              ceres_blocks_.getStaticCameraParameterBlockIntrinsics(all_cameras_[1]->camera_name_);
          P_BLOCK right_extrinsics =
              ceres_blocks_.getStaticCameraParameterBlockExtrinsics(all_cameras_[1]->camera_name_);
          P_BLOCK target_pb = ceres_blocks_.getMovingTargetPoseParameterBlock(target->target_name_, scene_);
          double lfx = left_intrinsics[0];
          double lfy = left_intrinsics[1];
          double lcx = left_intrinsics[2];
          ...
          for (int k = 0; k < all_targets_[0]->num_points_; k++)
          {
            ...
            Point3d right_point = target->pts_[right_camera_observations[k].point_id];
            CostFunction* cost_function = industrial_extrinsic_cal::WristStereoCal::Create(
                left_image_x, left_image_y, left_point, right_image_x, right_image_y, right_point, lfx, lfy, lcx, lcy,
                rfx, rfy, rcx, rcy);
            P_->AddResidualBlock(cost_function, NULL, target_pb, right_extrinsics);
          }  // for each point on the target
