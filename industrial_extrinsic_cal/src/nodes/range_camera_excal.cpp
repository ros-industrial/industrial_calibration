  ...
  : action_server_(nh_, "run_calibration", boost::bind(&RangeExCalService::actionCallback, this, _1), false)
{
  ...
  camera_ = shared_ptr<industrial_extrinsic_cal::Camera>(
      new industrial_extrinsic_cal::Camera(camera_name_, temp_parameters, false));
                                        ...
                                        industrial_extrinsic_cal::calibrate::Response& res)
{
  ...
  shared_ptr<industrial_extrinsic_cal::TransformInterface> temp_ti =
      shared_ptr<industrial_extrinsic_cal::ROSSimpleCameraCalTInterface>(
          new industrial_extrinsic_cal::ROSSimpleCameraCalTInterface(camera_frame_, camera_mounting_frame_));
  ...
  for (int i = 0; i < num_observations; i++)
  {
    ...
    pcl::PointXYZ pt3(pt1.x + pctx * (pt2.x - pt1.x), pt1.y + pcty * (pt2.y - pt1.y),
                      pt1.z + pcty * (pt2.z - pt1.z));  // interp along y between p1&p2

    ROS_INFO("image(%f %f) pff(%f %f %f), tpt(%f %f %f)", image_x, image_y, pff.x, pff.y, pff.z, tpoint.x, tpoint.y,
             tpoint.z);
