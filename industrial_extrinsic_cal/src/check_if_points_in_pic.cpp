...
int main(int argc, char** argv)
{
  ...
  initialization.center_point_of_target =
      initialization.finds_middle_of_target(initialization.center_Of_TargetX, initialization.center_Of_TargetY);
  ...
  geometry_msgs::PoseArray msg = initialization.create_all_poses(
      initialization.poseHeight, initialization.spacing_in_z, initialization.angleOfCone,
      initialization.numberOfStopsForPhotos, initialization.center_point_of_target);
  ...
  geometry_msgs::PoseArray filtered_msgs = initialization.pose_filters(
      msg2, initialization.tf_transform, initialization.image_width, initialization.image_height, AllcameraPoses,
      initialization.fx, initialization.fy, initialization.cx_, initialization.cy_);
...
void check_if_point_in_pic::imagePoint(Eigen::Vector3d TargetPoint, double fx, double fy, double cx, double cy,
                                       double& u, double& v, const Eigen::Affine3d& cameraPose)
{
  Eigen::Vector3d cp;
...
geometry_msgs::PoseArray check_if_point_in_pic::create_all_poses(double poseHeight, double spacing_in_z,
                                                                 double angleOfCone, int numberOfStopsForPhotos,
                                                                 Eigen::Vector2d center_point_of_target)
{
  ...
  {
    for (double l = (std::tan(angleInRadians)) * spacing_in_z; l <= (std::tan(angleInRadians) * (j));
         l = l + (std::tan(angleInRadians)) * spacing_in_z)
    {
...
geometry_msgs::PoseArray check_if_point_in_pic::pose_filters(geometry_msgs::PoseArray msg2,
                                                             tf::StampedTransform tf_transform, int image_width,
                                                             int image_height, EigenSTL::vector_Affine3d AllcameraPoses,
                                                             double fx, double fy, double cx, double cy)
{
  ...
  for (int j = 0; j < num_created_poses; j++)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      // ROS_INFO("%lf %lf %lf", tf_transform.getOrigin().x(), tf_transform.getOrigin().y(),
...
void check_if_point_in_pic::create_transform_listener(tf::StampedTransform& tf_transform,
...
check_if_point_in_pic::check_if_point_in_pic(ros::NodeHandle pivnh)
{
  ...
  string targetFile = "/home/lawrencelewis/catkin_ws/src/industrial_calibration/industrial_extrinsic_cal/yaml/"
                      "ical_srv_target.yaml";
  ...
  ROS_INFO("target name = %s num_points = %d size of pts_ = %d", Mytarget->target_name_, Mytarget->num_points_,
           (int)Mytarget->pts_.size());
