...
bool CalibrationJob::runObservations()
{
  ...
  BOOST_FOREACH(ObservationScene current_scene, scene_list_)
  {
	...
	if(target_type == Target::TARGET_TYPE::CIRCLEGRID){
	  circle_dia = observation.target->circle_dia;
	}
...
bool CalibrationJob::load()
{
  ...
  try
  {
	...
	if(temp_target->target_type == Target::TARGET_TYPE::CIRCLEGRID){
	  (*target_parameters)[i]["circle_dia"] >> temp_target->circle_diameter;
	}
...
bool CalibrationJob::runOptimization()
{
  ...
  BOOST_FOREACH(ObservationDataPoint ODP, observation_data_point_list_.items)
  {
    ...
    BOOST_FOREACH(ObservationDataPoint ODP, observation_data_point_list_.items){
      ...
      if(ODP.target_type == Target::TARGET_TYPE::CIRCLEGRID){ // this is for circle grids
	double circle_dia = ODP.circle_diameter;
