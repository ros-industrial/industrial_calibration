...
public:
    ...
    : nh_(nh), P_(NULL), problem_initialized_(false), total_observations_(0), scene_(0)
  {
    ...
    if (save_data_)
    {
      ...
      {
        ROS_ERROR("robocyl_ical_8D is saving image and rail position data in image_directory %s",
                  image_directory_.c_str());
      }
    }
    bool is_moving = true;
  ...
  bool loadCallBack(std_srvs::TriggerRequest& req, std_srvs::TriggerResponse& res)
  {
    ...
    sprintf(current_image_scene_chars, "_%03d_%03d.jpg", scene_,
            0);  // add rail distance index to image, scene_ will automatically be added by
                 // camera_observer.save_current_image()
    ...
    while (camera_->camera_observer_->exists_test(image_file_now))
    {
      ...
      // iether creates a place to hold all usefull information about images or groups all of the valuable information
      // into p_blocks
      ...
      for (int i = 0; i < (num_camera_locations_); i++)
      {
        ...
        sprintf(image_scene_chars, "_%03d_%03d.jpg", scene_,
                i);  // add rail distance index to image, scene_ will automatically be added by
                     // camera_observer.save_current_image()
    ...
    for (int i = 0; i < num_camera_locations_; i++)
    {
      ...
      else
      {
        ...
        for (int k = 0; k < num_observations; k++)
        {
          ...
          Point3d point = camera_observations[k].target->pts_[camera_observations[k].point_id];
          if (k == 10)
            ROS_ERROR("target point %d = %8.3lf %8.3lf %8.3lf observed at %8.3lf %8.3lf",
                      camera_observations[k].point_id, point.x, point.y, point.z, image_x, image_y);
          CostFunction* cost_function = industrial_extrinsic_cal::RailICal5::Create(image_x, image_y, Dist, point);
          P_->AddResidualBlock(cost_function, NULL, intrinsics, extrinsics, ax_ay_);
        }  // for each observation at this camera_location
      }    // end of else (there are some observations to add)
  ...
  bool computeCovariance(string& covariance_file_name)
  {
    ...
    if ((fp = fopen(covariance_file_name.c_str(), "w")) != NULL)
    {
      ...
      covariance_pairs.push_back(
          std::make_pair(camera_->camera_parameters_.pb_intrinsics, camera_->camera_parameters_.pb_intrinsics));
      ...
      if (covariance.Compute(covariance_pairs, P_))
      {
        ...
        if (covariance.GetCovarianceBlock(camera_->camera_parameters_.pb_intrinsics,
                                          camera_->camera_parameters_.pb_intrinsics, cov_in_in) &&
