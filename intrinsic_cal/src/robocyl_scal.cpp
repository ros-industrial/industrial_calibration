...
RobocylSCalService::RobocylSCalService(ros::NodeHandle nh)
{
  ...
  bool is_moving = true;
  left_camera_ = shared_ptr<industrial_extrinsic_cal::Camera>(
      new industrial_extrinsic_cal::Camera("left_camera", left_camera_parameters_, is_moving));
  right_camera_ = shared_ptr<industrial_extrinsic_cal::Camera>(
      new industrial_extrinsic_cal::Camera("right_camera", right_camera_parameters_, is_moving));
  ...
  left_camera_->camera_observer_ =
      shared_ptr<ROSCameraObserver>(new ROSCameraObserver(left_image_topic_, left_camera_name_));
  right_camera_->camera_observer_ =
      shared_ptr<ROSCameraObserver>(new ROSCameraObserver(right_image_topic_, right_camera_name_));
  sleep(10);  // wait for camera to come up or else this will fail
  if (!left_camera_->camera_observer_->pullCameraInfo(
          left_camera_->camera_parameters_.focal_length_x, left_camera_->camera_parameters_.focal_length_y,
          left_camera_->camera_parameters_.center_x, left_camera_->camera_parameters_.center_y,
          left_camera_->camera_parameters_.distortion_k1, left_camera_->camera_parameters_.distortion_k2,
          left_camera_->camera_parameters_.distortion_k3, left_camera_->camera_parameters_.distortion_p1,
          left_camera_->camera_parameters_.distortion_p2, left_image_width_, left_image_height_))
  {
    ROS_FATAL("Could not get camera information for %s from topic %s. Shutting down node.", left_camera_name_.c_str(),
              left_image_topic_.c_str());
    ros::shutdown();
  }

  if (!right_camera_->camera_observer_->pullCameraInfo(
          right_camera_->camera_parameters_.focal_length_x, right_camera_->camera_parameters_.focal_length_y,
          right_camera_->camera_parameters_.center_x, right_camera_->camera_parameters_.center_y,
          right_camera_->camera_parameters_.distortion_k1, right_camera_->camera_parameters_.distortion_k2,
          right_camera_->camera_parameters_.distortion_k3, right_camera_->camera_parameters_.distortion_p1,
          right_camera_->camera_parameters_.distortion_p2, right_image_width_, right_image_height_))
  {
    ROS_FATAL("Could not get camera information for %s from topic %s. Shutting down node.", right_camera_name_.c_str(),
...
bool RobocylSCalService::executeCallBack(intrinsic_cal::rail_scal_run::Request& req,
                                         intrinsic_cal::rail_scal_run::Response& res)
{
  ...
  int total_observations =
      2 * num_camera_locations_ * num_observations;  // each point observed twice, once by each camera in every position
  ...
  Pose6d LC2toLC =
      TtoLC * LC2toT;  // this transforms points in LC2 frame into Target then into LC frame as one multiply
    ...
    if (left_ok && right_ok)
    {
      ...
      {  // add a new cost to the problem for each observation
        for (int j = 0; j < num_observations; j++)
        {
          ...
          Point3d left_point =
              target_->pts_[left_camera_observations[j].point_id];  // don't assume ordering from camera observer
          double right_image_x = right_camera_observations[j].image_loc_x;
          double right_image_y = right_camera_observations[j].image_loc_y;
          Point3d right_point =
              target_->pts_[right_camera_observations[j].point_id];  // don't assume ordering from camera observer
          cost_function[q] = industrial_extrinsic_cal::RailSCal::Create(
              left_image_x, left_image_y, left_point, right_image_x, right_image_y, right_point, rail_position, lfx_,
              lfy_, lcx_, lcy_, lk1_, lk2_, lk3_, lp1_, lp2_, rfx_, rfy_, rcx_, rcy_, rk1_, rk2_, rk3_, rp1_, rp2_);
          problem.AddResidualBlock(cost_function[q], NULL, LCtoRC.pb_pose,
  ...
  if (summary.termination_type != ceres::NO_CONVERGENCE)
  {
    ...
    if (final_cost <= req.allowable_cost_per_observation)
    {
      ...
      target_->pose_.getQuaternion(res.right_camera_pose.orientation.x, res.right_camera_pose.orientation.y,
                                   res.right_camera_pose.orientation.z, res.right_camera_pose.orientation.w);
      return true;
    }
    else
    {
...
bool RobocylSCalService::MoveAndReportPose(double rail_position, Pose6d& LP, Pose6d& RP)
{
  ...
  if (left_obs_ok && right_obs_ok)
  {
    ...
    for (int i = 0; i < num_observations; i++)
    {
      ...
      Point3d left_point =
          target_->pts_[left_camera_observations[i].point_id];  // don't assume ordering from camera observer
      cost_function[q] = industrial_extrinsic_cal::DistortedCameraFinder::Create(
          left_image_x, left_image_y, lfx_, lfy_, lcx_, lcy_, lk1_, lk2_, lk3_, lp1_, lp2_, left_point);
      ...
      Point3d right_point =
          target_->pts_[right_camera_observations[i].point_id];  // don't assume ordering from camera observer
      cost_function[q] = industrial_extrinsic_cal::DistortedCameraFinder::Create(
          right_image_x, right_image_y, rfx_, rfy_, rcx_, rcy_, rk1_, rk2_, rk3_, rp1_, rp2_, right_point);
