...
public:
    ...
    for (int i = 0; i < all_cameras_.size(); i++)
    {
      ...
      {  // add a new cost to the problem for each observation
        ...
        for (int k = 0; k < num_observations; k++)
        {
          ...
          cost_function[k] =
              industrial_extrinsic_cal::CameraReprjErrorWithDistortionPK::Create(image_x, image_y, point);
          if (target_pb == NULL)
          {
            ROS_ERROR("ical_srv attempting to add cost with target_pb = NULL perhaps you don't have a moving target "
                      "defined");
            continue;
          }
          else if (intrinsics == NULL)
          {
            ROS_ERROR("ical_srv attmpting to add cost with intrinsics = NULL, perhaps you don't have a static camera "
                      "defined");
            continue;
          }
  ...
  bool loadCallBack(std_srvs::TriggerRequest& req, std_srvs::TriggerResponse& res)
  {
    ...
    // Note that the target's transform interface should provide the pose between the target and the camera that
    // observes it usually there is only one camera, and one target
    ...
    {  // read all the scene data from image_directory
      ...
      if (data_read_ok)
      {
        for (int i = 0; i < all_cameras_.size(); i++)
        {
          ...
          for (int j = 0; j < all_targets_.size(); j++)
          {
            all_cameras_[i]->camera_observer_->addTarget(
                all_targets_[j], roi, industrial_extrinsic_cal::cost_functions::CameraReprjErrorWithDistortionPK);
          }
