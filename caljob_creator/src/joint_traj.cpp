...
public:
  // constructor
  JointTraj(ros::NodeHandle nh) : nh_(nh), current_index_(-1)
  {
...
private:
  ...
  bool MoveNextCallBack(std_srvs::TriggerRequest& req, std_srvs::TriggerResponse& res)
  {
    ...
    char index_msg[100];
    sprintf(index_msg, "%d",
            current_index_);  // because stings are soooo easy to use just like cout<< and other breaks make by cs
    if (moveIndex(index))
    {
  ...
  bool ExecuteWTriggerCallBack(std_srvs::TriggerRequest& req, std_srvs::TriggerResponse& res)
  {
    ...
    {
      char index_msg[100];
      sprintf(index_msg, "%d data had %d poses", current_index_,
              (int)joint_trajectory_data.size());  // because stings are soooo easy to use just like cout<< and other
                                                   // breaks make by cs
      res.message = std::string("Failed to execute complete path current_index_ = ") + index_msg;
      res.success = false;
      return (true);
