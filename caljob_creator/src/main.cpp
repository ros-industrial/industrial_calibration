...
public:
  ...
  {
    write_joint_scene(file_, scene_id_++, state.position);
  }

  void new_pose_scene(const tf::StampedTransform& transform)
  {
    write_pose_scene(file_, scene_id_++, transform);
  }
...
private:
  ...
  void write_joint_scene(std::ostream& os, std::size_t id, const std::vector<double>& joints) const
  {
    os << "-\n";
    os << "     scene_id: " << id << '\n';
  ...
  void write_pose_scene(std::ostream& os, std::size_t id, tf::StampedTransform transform) const
  {
    os << "-\n";
    os << "     scene_id: " << id << '\n';
...
int main(int argc, char** argv)
{
  ...
  nh.param<int>("image_topic", image_height, 480);
